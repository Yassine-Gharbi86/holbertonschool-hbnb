document.addEventListener('DOMContentLoaded', () => {
    ...
    // Check for review form and handle review submission
    if (reviewForm) {
      const token = checkAuthentication();
      const placeId = getPlaceIdFromURL();
      
      if (!placeId) {
        alert('No place ID found in URL. Redirecting to homepage.');
        window.location.href = 'index.html';
        return;
      }
  ...
  function checkAuthentication() {
    ...
    if (!token) {
      // For review pages, redirect to index if not authenticated
  ...
  // New functions to handle review submission
  
  /**
   * Submit a review for a place
   * @param {string} token - The JWT authentication token
   * @param {string} placeId - The ID of the place to review
  ...
  async function handleReviewResponse(response, form) {
    try {
      ...
      alert('An error occurred while processing your review. Please try again.');
    }
  }